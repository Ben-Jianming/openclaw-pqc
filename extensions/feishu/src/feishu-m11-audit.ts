...
function getOrCreateEd25519Key(env: NodeJS.ProcessEnv): { secretRaw: Uint8Array; keyId: string } {
  ...
  const keyId = createHash("sha256").update(Buffer.from(secretRaw)).digest("hex").slice(0, 16);
  return { secretRaw, keyId };
}

function getOrCreateMldsaKey(env: NodeJS.ProcessEnv): {
  ...
  if (existsSync(filePath)) {
    ...
    publicRaw = new Uint8Array(
      buf.subarray(MLDSA65_RAW_SECRET, MLDSA65_RAW_SECRET + MLDSA65_RAW_PUBLIC),
    );
  } else {
