...
import { hmac as hmacN } from "@noble/hashes/hmac.js";
...
export class NobleCryptoProvider implements CryptoProvider {
  ...
  async kemGenerate(alg: KemAlg): Promise<{ publicKey: Key; privateKey: Key }> {
    if (alg === "ml-kem-768") {
      ...
      return {
        publicKey: { alg, material: k.publicKey, kind: "public" },
        privateKey: { alg, material: k.secretKey, kind: "private" },
      };
    }
    throw new UnsupportedAlgorithmError(alg);
  }
  async kemEncapsulate(
    ...
    if (alg === "ml-kem-768") {
      const r = ml_kem768.encapsulate(pk.material);
