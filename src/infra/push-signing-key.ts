...
export function resolvePushSigningKeyPath(env: NodeJS.ProcessEnv = process.env): string {
  ...
  if (fromEnv && fromEnv.length > 0) {
    if (!isAbsolute(fromEnv)) {
      throw new Error(`${PUSH_SIGNING_KEY_FILE_ENV} must be an absolute path; got ${fromEnv}`);
    }
    return fromEnv;
  }
 ...
 */
export function getOrCreatePushSigningKey(env: NodeJS.ProcessEnv = process.env): PushSigningKey {
